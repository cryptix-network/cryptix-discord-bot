...
async def change_status():
    statuses = [
        ...
        discord.Activity(type=discord.ActivityType.playing, name="with CryptixX!"),
        discord.Activity(type=discord.ActivityType.listening, name="miners' chatter!"),
        discord.Activity(
            type=discord.ActivityType.watching, name="block height increases!"
        ),
        discord.Activity(
            type=discord.ActivityType.playing, name="cryptographic algorithms!"
        ),
        discord.Activity(
            type=discord.ActivityType.watching, name="the blockchain grow!"
        ),
        discord.Activity(type=discord.ActivityType.playing, name="the hash war game!"),
        discord.Activity(
            type=discord.ActivityType.competing, name="solving the next block!"
        ),
    ]
...
async def check_display_name(member):
    ...
    for keyword in flagged_keywords:
        if keyword.lower() in member.display_name.lower():
            logging.debug(
                f"Flagged keyword found in display name: {member.display_name}"
            )
            await handle_suspicious_change(
                member, f"Display name contains flagged keyword: {keyword}"
            )
            break
    ...
    if len(user_history) == SPAM_THRESHOLD and all(
        ...
        if (
            ...
            timeout_message = await message.channel.send(
                f"{message.author.mention} You have been temporarily muted for 15 minutes due to spamming."
            )
...
async def handle_banned_keyword(message, keyword):
    ...
    await message.guild.ban(
        message.author, reason=f"Message contained banned content: {keyword}"
    )
    logging.warning(
        f"Banned {message.author.name} for sending a message with banned content (ID: {message.author.id}), keyword: {keyword}"
    )
    await log_action(
        message.guild,
        f"Banned {message.author.name} for sending a message with banned content (ID: {message.author.id}), keyword: {keyword}",
...
async def set_category_name():
    ...
    if guild:
        ...
        if category:
            await category.edit(name="--Cryptix Network Stats--")
            logging.info("Category name set to --Cryptix Network Stats--")
    ...
    if guild:
        while True:
            try:
                ...
                await update_channel(
                    guild, "Next Reward:", get_halving_data, next_reward=True
                )
                await update_channel(
                    guild, "Next Reduction:", get_halving_data, next_reduction=True
                )
                await update_channel(guild, "Price", get_price_data)
                await update_channel(guild, "mcap", get_market_cap)
                await update_channel(
                    guild, "24h Volume:", get_24h_volume, is_volume=True
                )
...
async def get_max_supply():
    async with aiohttp.ClientSession() as session:
        async with session.get(
            "https://api.cryptix-network.org/info/coinsupply/max",
...
async def get_24h_volume():
    url = "https://api.coingecko.com/api/v3/coins/cryptix-network"
    async with aiohttp.ClientSession() as session:
...
async def get_circulating_supply():
    async with aiohttp.ClientSession() as session:
        async with session.get(
            "https://api.cryptix-network.org/info/coinsupply/circulating?in_billion=false",
            headers={"accept": "text/plain"},
        ) as response:
...
async def get_hashrate():
    async with aiohttp.ClientSession() as session:
        async with session.get(
            "https://api.cryptix-network.org/info/hashrate?stringOnly=false",
...
async def get_blockreward():
    async with aiohttp.ClientSession() as session:
        async with session.get(
            "https://api.cryptix-network.org/info/blockreward?stringOnly=false",
            headers={"accept": "application/json"},
        ) as response:
...
async def get_halving_data():
    async with aiohttp.ClientSession() as session:
        async with session.get(
            "https://api.cryptix-network.org/info/halving",
            headers={"accept": "application/json"},
        ) as response:
...
async def get_price_data():
    async with aiohttp.ClientSession() as session:
        async with session.get(
            "https://api.cryptix-network.org/info/price?stringOnly=false",
            headers={"accept": "application/json"},
        ) as response:
...
async def get_market_cap():
    async with aiohttp.ClientSession() as session:
        async with session.get(
            "https://api.cryptix-network.org/info/marketcap?stringOnly=false",
            headers={"accept": "application/json"},
        ) as response:
...
async def calc(ctx, hashrate: float = None):
    ...
    blockreward, network_hashrate_ths, cytx_price = await fetch_network_info_and_price()
    if (
        blockreward is not None
        and network_hashrate_ths is not None
        and cytx_price is not None
    ):
        ...
        total_CYTX_per_day = blocks_per_day * blockreward
        emissions24hr = total_CYTX_per_day * cytx_price

        reward_message = (
            f"**Network Hashrate:** {network_hashrate_mhs:.2f} MH/s\n"
            f"**Daily CYTX Mined:** {total_CYTX_per_day:.2f} CYTX\n"
            f"**24h Emissions:** {emissions24hr:.2f} USD\n"
            f"**Block Reward:** {blockreward} CYTX\n"
            f"**Price per CYTX:** {cytx_price:.5f} USD\n"
        ...
        for period, reward in rewards.items():
            profit_usd = reward * cytx_price
            reward_message += f"- {period}: {reward:.2f} CYTX ({profit_usd:.3f} USD)\n"

        await ctx.send(reward_message)
    else:
        await ctx.send(
            "Failed to retrieve network information or CYTX price. Please try again later."
        )
...
async def fetch_network_info_and_price():
    try:
        async with aiohttp.ClientSession() as session:
            blockreward_response = await session.get(
                "https://api.cryptix-network.org/info/blockreward?stringOnly=false",
                headers={"accept": "application/json"},
            )
            blockreward = (await blockreward_response.json()).get("blockreward")

            hashrate_response = await session.get(
                "https://api.cryptix-network.org/info/hashrate?stringOnly=false",
                headers={"accept": "application/json"},
            )
            network_hashrate = (await hashrate_response.json()).get(
                "hashrate"
            )  # in TH/s
            ...
            cytx_price = (await price_response.json()).get("price")  # price in USD

            return blockreward, network_hashrate, cytx_price
    except Exception as e:
...
def get_mining_rewards(blockreward, percent_of_network):
    ...
    rewards["Week"] = (
        rewards_in_range(blockreward, 60 * 60 * 24 * 7) * percent_of_network
    )
    rewards["Month"] = (
        rewards_in_range(blockreward, 60 * 60 * 24 * (365.25 / 12)) * percent_of_network
    )
    rewards["Year"] = (
        rewards_in_range(blockreward, 60 * 60 * 24 * 365.25) * percent_of_network
    )
    return rewards
